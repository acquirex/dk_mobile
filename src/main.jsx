...
import { Search, Menu, ShoppingCart, ChevronRight, Heart, Home, Grid2X2, Headphones, ShieldCheck, Truck, RotateCcw, Star, ArrowRight, ArrowLeft, SlidersHorizontal, ChevronDown, Sparkles, FileText, X, Camera, Send, Upload, Phone, Mail, MessageCircle } from 'lucide-react';
import './styles.css';
...
const products = [
  ...
  { name: 'CRL 1006D73 Bronze 73” Outswing Adjustable Oak Top Threshold', price: '$327.20', old: '', image: '/products/crl-1006d73.png', tag: 'LIMITED STOCK', meta: 'Item #1006D73 · CRL' },
  ...
  { name: 'Precision Replacement Molding Clip — Pack of 25', price: '$9.94', old: '$21.50', image: 'https://images.unsplash.com/photo-1504148455328-c376907d081c?auto=format&fit=crop&w=600&q=80', tag: 'SAVE 53%' },
...
const bathPopular = [
  ...
  ['Deep U-Channel for 3/8” and 1/2” Glass', 'https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?auto=format&fit=crop&w=500&q=80'],
...
const bathProducts = [
  ['Clear Polycarbonate H-Jamb 180 Degree for 3/8” Glass', '$25.57', bathPopular[0][1]],
  ...
  ['Multi-Purpose Clear L Angle Jamb Seal — 95”', '$16.87', bathPopular[3][1]],
...
const promotions = [
  ...
  { eyebrow: 'CUSTOM QUOTES', title: 'Better pricing for bigger jobs.', copy: 'Send your list. We’ll handle the quote.', action: 'Request a quote', image: '/banners/custom-quotes.jpg', tone: 'blue' },
...
function SiteFooter() {
  ...
  return <footer className="site-footer"><div className="footer-brand"><img src="https://dkstatic.blob.core.windows.net/static/dkh/dkhardware-logo.svg" alt="DK Hardware"/><p>Hardware solutions for every project.</p></div><div className="footer-contact"><a href="tel:+18775098040"><Phone/><span><b>Call us</b><small>+1 877 509 8040</small></span></a><a href="mailto:sales@dkhardware.com"><Mail/><span><b>Email us</b><small>Sales and support</small></span></a></div><form className="footer-signup" onSubmit={event=>{event.preventDefault();triggerPrototype('Thanks — you are signed up')}}><h3>Get deals and product updates</h3><div><input required type="email" placeholder="Email address" aria-label="Email address"/><button>Sign up</button></div></form><div className="footer-links">{groups.map(([title,links])=><details key={title}><summary>{title}<ChevronDown/></summary><div>{links.map(link=><a href="#" onClick={event=>{event.preventDefault();triggerPrototype(`${link} selected`,link==='Request a quote'?'quote':null)}} key={link}>{link}</a>)}</div></details>)}</div><div className="footer-bottom"><span>© 2026 DK Hardware Supply</span><span>Secure payments</span></div></footer>;
}

function AppNav({ active = 'home', onHome, onCart, onQuote, cartCount = 0 }) {
  ...
  return <>
    ...
    {panel === 'ai' && <aside className="app-tool-panel ai-panel">
      ...
      <div className="ai-conversation">{asked ? <><div className="user-message">{question || 'Help me identify this hardware.'}{fileName && <small>📎 {fileName}</small>}</div><div className="ai-message"><Sparkles/><p>I can help identify parts, compare products, or plan your project. This prototype is ready to connect to the DK AI service.</p></div></> : <div className="ai-welcome"><Sparkles/><h3>Ask DK Hardware AI</h3><p>Find a part, solve a project problem, or upload a photo to identify hardware.</p><div><button onClick={() => setQuestion('What hardware do I need for a frameless shower door?')}>Plan a project</button><button onClick={() => setQuestion('Help me find a replacement part')}>Find a part</button></div></div>}</div>
    ...
    {panel === 'menu' && <aside className="app-tool-panel support-panel menu-panel"><div className="tool-panel-head"><div><h2>Browse DK Hardware</h2></div><button onClick={()=>setPanel(null)}><X/></button></div><div className="support-actions"><button onClick={()=>{setPanel(null);if(onHome)onHome()}}><Grid2X2/><span><b>Shop departments</b><small>Browse hardware categories</small></span><ChevronRight/></button><button onClick={()=>{setPanel(null);if(onQuote)onQuote();else setPanel('quote')}}><FileText/><span><b>Request a quote</b><small>Volume and project pricing</small></span><ChevronRight/></button><button onClick={()=>setPanel('support')}><Headphones/><span><b>Customer support</b><small>Talk to a hardware specialist</small></span><ChevronRight/></button><button onClick={()=>setPanel('ai')}><Sparkles/><span><b>Ask DK AI</b><small>Find parts and project help</small></span><ChevronRight/></button></div></aside>}
    {notice&&<div className="prototype-toast">{notice}</div>}
    <nav className="bottom-nav"><button className={active==='home'?'active':''} onClick={onHome}><Home/><span>Home</span></button><button className={active==='shop'?'active':''} onClick={onHome}><Grid2X2/><span>Shop</span></button><button className={active==='quote'?'active':''} onClick={onQuote||(()=>setPanel('quote'))}><FileText/><span>Get Quote</span></button><button onClick={()=>setPanel('support')}><Headphones/><span>Support</span></button><button onClick={onCart||(()=>setNotice('Your cart is empty'))} className={active==='cart'?'active':''}><span className="nav-cart-icon"><ShoppingCart/>{cartCount>0&&<i>{cartCount}</i>}</span><span>Cart</span></button></nav>
  </>;
}
...
function ShowerKnobsPage({ onBack, onHome, onQuote }) {
  ...
  return <main className="stage">
    <div className="phone-shell"><div className="speaker"></div><div className="app">
      ...
      {commerceStep==='added'&&justAdded&&<AddedCartPanel product={justAdded} recommendations={showerKnobProducts.filter(product=>product.item!==justAdded.item&&!cartItems.includes(product.item)).slice(0,2)} subtotal={cartTotal} onClose={()=>setCommerceStep(null)} onViewCart={()=>setCommerceStep('cart')} onCheckout={()=>setCommerceStep('checkout')} onAdd={toggleCart}/>}
...
function App() {
  ...
  if (page === 'shower-knobs') return <ShowerKnobsPage onBack={() => setPage('shower-handles')} onHome={() => setPage('home')} onQuote={()=>setPage('quote')}/>;
      ...
      <div className="app">
        ...
        <div className="app-content">
          ...
          <section className="audience">
            <div className="section-title audience-title"><h2>Fix it, build it, boss it — <em>DK delivers it!</em></h2></div>
          ...
          <section className="trust"><ShieldCheck/><div><h2>Trust us—we've got you covered.</h2><p>Five-star service, secure payments and expert support.</p></div></section>
        ...
        <AppNav active={homeCommerceStep==='cart'?'cart':'home'} onHome={() => setPage('home')} onCart={()=>setHomeCommerceStep('cart')} onQuote={()=>setPage('quote')} cartCount={homeCartCount}/>
      </div>
    </div>
  </main>
}
